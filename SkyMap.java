...
public class SkyMap {
    ...
    public double J2000(double Y, double M, double D, double TIME) {
        double JD = (367 * Y - floor(7 * (Y + floor((M + 9) / 12)) / 4)
                - floor(3 * (floor((Y + (M - 9) / 7) / 100) + 1) / 4) + floor(275 * M / 9) + D + 1721028.5 + TIME / 24);
     ...
     */
    public double J2000() {
        _JD = (367 * _year - floor(7 * (_year + floor((_month + 9) / 12)) / 4)
                - floor(3 * (floor((_year + (_month - 9) / 7) / 100) + 1) / 4) + floor(275 * _month / 9) + _day
                + 1721028.5 + _time / 24);
    ...
    public SearchResult calculate_AZ_alt(double hour_angle, double declination, double lattitude) {
        ...
        double sinDEC = sin(deg2rad(declination));
        double sinHA = sin(deg2rad(hour_angle));
        double sinLAT = sin(deg2rad(lattitude));
        double cosDEC = cos(deg2rad(declination));
        double cosHA = cos(deg2rad(hour_angle));
        double cosLAT = cos(deg2rad(lattitude));
